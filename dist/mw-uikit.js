...
angular.module("mwUI").run(["$templateCache", function($templateCache) {  'use strict';
  ...
  $templateCache.put('uikit/mw-layout/directives/templates/mw_header.html',
    "<div class=\"mw-header row\"><div class=\"fixed-content col-md-12\"><div class=\"title-holder\"><div class=\"title-holder-item\"><span mw-icon=\"{{mwTitleIcon}}\" class=\"header-icon\" ng-if=\"mwTitleIcon\"></span><div ng-if=\"mwBreadCrumbs\" mw-bread-crumbs-holder><div ng-repeat=\"breadCrumb in mwBreadCrumbs\" mw-bread-crumb url=\"{{breadCrumb.url}}\" title=\"{{breadCrumb.title}}\" show-left-arrow=\"canShowBackButton() && $index === 0\"></div></div><div><h1 class=\"lead page-title\"><span class=\"text\">{{title}}</span></h1></div></div><div ng-if=\"showDescription\" class=\"title-holder-item\"><div class=\"text description\">{{description}}</div></div></div><div class=\"action-btn-bar\"><div ng-if=\"canShowDescriptionButton()\" class=\"action-btn clickable round-btn description-btn\" ng-click=\"toggleDescription()\"><span mw-icon=\"mwUI.info\"></span></div><div ng-if=\"showRefreshButton\" class=\"action-btn clickable round-btn\" ng-click=\"refresh()\"><span mw-icon=\"mwUI.refresh\"></span></div></div><div class=\"additional-content-holder\" ng-transclude></div></div></div>"
  );
  ...
  $templateCache.put('uikit/mw-list/directives/templates/mw_list_column_configurator.html',
    "<div class=\"mw-list-column-configurator btn-group\"><button type=\"button\" class=\"btn btn-default btn-xs dropdown-toggle\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\"><span mw-icon=\"mwUI.gears\"></span> <span class=\"caret\"></span></button><ul class=\"dropdown-menu\"><li ng-repeat=\"column in columns\" ng-if=\"column.scope && getColTitle(column)\" ng-class=\"{'is-checked': column.scope.isVisible(), disabled: column.scope.isMandatory()}\"><a mw-prevent-default=\"click\" ng-click=\"toggleColumn(column)\" mw-stop-propagation=\"click\"><span mw-icon=\"mwUI.check\" class=\"checked-indicator\"></span> {{getColTitle(column)}}</a></li><li class=\"divider\"></li><li><a mw-prevent-default=\"click\" mw-stop-propagation=\"click\" ng-click=\"reset()\"><span mw-icon=\"mwUI.undo\"></span> Reset</a></li></ul></div>"
  );
  ...
  $templateCache.put('uikit/mw-menu/directives/templates/mw_menu_top_drop_down_item.html',
    "<div class=\"mw-menu-top-drop-down-item dropdown\"><a class=\"dropdown-toggle\" data-toggle=\"dropdown\" role=\"button\" aria-haspopup=\"true\" aria-expanded=\"false\" mw-menu-toggle-active-class=\"entry\" id=\"{{entry.get('id')}}\"><span mw-icon=\"{{entry.get('icon')}}\"></span> <span>{{entry.get('label')}}</span> <b class=\"caret\"></b></a><ul class=\"dropdown-menu\"><li ng-repeat=\"subEntry in entry.get('subEntries').models\" mw-menu-top-item=\"subEntry\"></li></ul></div>"
  );


  $templateCache.put('uikit/mw-menu/directives/templates/mw_menu_top_entries.html',
    "<ul class=\"mw-menu-top-entries nav navbar-nav\" ng-class=\"{'navbar-right': right}\"><li ng-repeat=\"entry in entries.models\" ng-if=\"entry.get('isVisible')\" class=\"entry {{entry.get('class')}}\"><div ng-if=\"entry.hasSubEntries()\" mw-menu-top-drop-down-item=\"entry\"></div><div ng-if=\"!entry.hasSubEntries()\" mw-menu-top-item=\"entry\"></div></li></ul><div ng-transclude hidden class=\"mw-menu-entries\"></div>"
  );


  $templateCache.put('uikit/mw-menu/directives/templates/mw_menu_top_item.html',
    "<div class=\"mw-menu-top-item\" ng-if=\"entry.get('isVisible')\" ng-click=\"executeAction()\"><a ng-if=\"entry.get('type') === 'ENTRY'\" ng-href=\"{{entry.get('url')}}\" target=\"{{entry.get('target')}}\" mw-menu-toggle-active-class=\"entry\" id=\"{{entry.get('id')}}\" class=\"entry\"><span ng-if=\"entry.get('icon')\" mw-icon=\"{{entry.get('icon')}}\"></span> <span>{{entry.get('label')}}</span></a><div ng-if=\"entry.get('type') === 'DIVIDER'\" class=\"divider\" title=\"{{entry.get('label')}}\"></div></div>"
  );
  ...
  $templateCache.put('uikit/mw-modal/directives/templates/mw_modal_confirm.html',
    "<div mw-modal title=\"{{ 'Modal.mwModalConfirm.areYouSure' | i18n }}\"><div mw-modal-body><div ng-transclude></div></div><div mw-modal-footer><button type=\"button\" class=\"btn btn-default\" data-dismiss=\"modal\" ng-click=\"cancel()\" id=\"mw_btn_cancel\">{{'Utils.cancel' | i18n }}</button> <button type=\"button\" mw-modal-on-enter class=\"btn btn-primary\" data-dismiss=\"modal\" ng-click=\"ok()\" id=\"mw_btn_confirm\">{{'Utils.ok' | i18n }}</button></div></div>"
  );
  ...
  $templateCache.put('uikit/mw-toast/directives/templates/mw_toasts.html',
    "<div class=\"message messages-list mw-toasts\"><div class=\"content\"><ul><li ng-repeat=\"toast in toasts\" class=\"message-item\"><div class=\"status-indicator {{toast.type}}\"><span mw-icon=\"{{toast.icon}}\"></span></div><div class=\"message {{toast.type}}\"><div class=\"holder margin-top-5\"><h5 ng-if=\"toast.title\">{{toast.title}}</h5><span ng-if=\"!toast.isHtmlMessage\">{{toast.message | limitTo:500}}</span> <span ng-if=\"toast.isHtmlMessage\" ng-bind-html=\"getHtmlMessage(toast.message)\"></span> <a class=\"action-button btn btn-link btn-xs\" ng-if=\"toast.button && toast.button.isLink && toast.button.action && !toast.button.link\"><span ng-click=\"hideToast(toast); toast.button.action()\" mw-prevent-default=\"click\">{{toast.button.title}}</span></a> <a class=\"action-button btn btn-link btn-xs\" ng-if=\"toast.button && toast.button.isLink && toast.button.link\" ng-href=\"{{toast.button.link}}\" target=\"{{toast.button.target}}\"><span>{{toast.button.title}}</span></a><div ng-if=\"toast.button && !toast.button.isLink && toast.button.action\"><div class=\"action-button btn btn-default btn-xs margin-top-5\"><div ng-click=\"hideToast(toast); toast.button.action()\">{{toast.button.title}}</div></div></div></div><div class=\"closer {{toast.type}}\" ng-click=\"hideToast(toast.id)\"><span mw-icon=\"mwUI.cross\"></span></div></div></li></ul></div></div>"
  );
  ...
  $templateCache.put('uikit/mw-ui-components/directives/templates/mw_bread_crumb.html',
    "<div class=\"mw-bread-crumb\"><a ng-href=\"{{url}}\" class=\"bread-crumb\" ng-class=\"{'has-left-arrow': showLeftArrow}\"><span ng-if=\"showLeftArrow\" class=\"arrow-left\" mw-icon=\"mwUI.angleLeft\"></span> {{title}}</a> <span mw-icon=\"mwUI.caretRight\" class=\"arrow-right\"></span></div>"
  );
  ...
  $templateCache.put('uikit/mw-ui-components/directives/templates/mw_indefinite_loading.html',
    "<div class=\"mw-infinite-loading row\"><div class=\"col-md-12 text-center\"><div mw-spinner></div><div class=\"lead\">{{'UiComponents.mwIndefiniteLoading.loading' | i18n | uppercase}}</div></div></div>"
  );


  $templateCache.put('uikit/mw-ui-components/directives/templates/mw_option_group.html',
    "<div class=\"mw-option-group panel panel-default\"><fieldset ng-disabled=\"mwDisabled\"><div class=\"panel-body\"><span ng-transclude></span><label class=\"options-container display-inline clickable\" ng-class=\"{'with-icon':icon}\" for=\"{{randomId}}\" ng-click=\"select()\"><div class=\"clearfix\"><div ng-if=\"icon\" class=\"col-md-1 icon-holder\"><span mw-icon=\"{{icon}}\"></span></div><div class=\"description\" ng-class=\"{'col-md-11': icon, 'col-md-12': !icon}\"><div class=\"title\"><h4>{{title}}</h4><div ng-if=\"badges\" class=\"badges\"><div mw-badge=\"info\" ng-repeat=\"badge in badges\">{{badge}}</div></div></div><p ng-if=\"description\">{{description}}</p></div></div></label></div></fieldset></div>"
  );
  ...
  $templateCache.put('uikit/mw-ui-components/directives/templates/mw_tab_bar.html',
    "<div class=\"clearfix mw-tab-bar\" ng-class=\"{justified: justified}\"><ul class=\"nav nav-tabs\" ng-class=\"{ 'nav-justified': justified }\"><li ng-repeat=\"pane in panes\" ng-class=\"{ active: $index === getActivePaneIndex() }\"><a ng-class=\"{ 'has-error': pane.isInvalid }\" ng-click=\"select(pane);\" id=\"{{'tab:' + pane.id}}\" mw-prevent-default=\"click\" mw-stop-propagation=\"click\"><span ng-if=\"pane.icon\" mw-icon=\"{{pane.icon}}\"></span> {{ pane.title }} <span ng-if=\"pane.badge\" class=\"badge badge-default badge-light\">{{pane.badge}}</span> <span ng-if=\"pane.tooltip\" mw-icon=\"mwUI.questionCircle\" tooltip=\"{{pane.tooltip}}\"></span></a></li></ul><div ng-show=\"canShowPaneContent()\" class=\"tab-content\" ng-transclude></div><div ng-if=\"!canShowPaneContent()\" mw-indefinite-loading></div></div>"
  );


  $templateCache.put('uikit/mw-ui-components/directives/templates/mw_tab_pane.html',
    "<div class=\"tab-pane mw-tab-pane\" role=\"tabpanel\" ng-class=\"{active: isSelected()}\"><div ng-if=\"canUseNgIf()\"><div ng-if=\"isSelected()\" ng-transclude></div></div><div ng-if=\"!canUseNgIf()\"><div ng-show=\"isSelected()\" ng-transclude></div></div></div>"
  );
  ...
  $templateCache.put('uikit/mw-list/i18n/en_US.json',
    "{ \"List\": { \"mwListHead\": { \"items\": \"Items\", \"selectAll\": \"Select all\", \"clearSelection\": \"Clear selection\", \"itemSelected\": \"{{name}} is selected\", \"itemsSelected\": \"{{count}} out of {{total}} {{name}} are selected\", \"itemAmount\": \"{{total}} {{name}}\", \"searchFor\": \"Search for {{name}}\", \"notAvailable\": \"N/V\", \"notAvailableTooltip\": \"The entry is not available anymore. Maybe is has been deleted.\" }, \"mwListFooter\": { \"noneFound\": \"No entries have been found.\" } } }"
  );
...
angular.module('mwUI.Menu')
  ...
  .directive('mwMenuTopItem', ['$timeout', function ($timeout) {
    return {
      scope: {
        entry: '=mwMenuTopItem'
      },
...
angular.module('mwUI.Modal')
  ...
  .service('Modal', ['$rootScope', '$location', '$templateCache', '$document', '$compile', '$controller', '$injector', '$q', '$templateRequest', '$timeout', 'mwModalOptions', 'Toast', function ($rootScope, $location, $templateCache, $document, $compile, $controller, $injector, $q, $templateRequest, $timeout, mwModalOptions, Toast) {
...
angular.module('mwUI.UiComponents')
  ...
  .directive('mwOptionGroup', ['$timeout', function ($timeout) {
    return {
      scope: {
        title: '@',
        description: '@?',
 ...
 * ```
 */
  .directive('mwTabs', ['$rootScope', function ($rootScope) {
    return {
      transclude: true,
      scope: {
        justified: '=?',
