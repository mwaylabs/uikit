    ...
    shell: {
      ...
      gitRelease: {
        ...
        command: function(){
          return [
            './bin/git_release.sh'
            //'git commit -m "release build v' + uikitConfig.getVersionWithBuildNumber()+'"',
            //'git push --force --quiet "https://${GH_TOKEN}@${GH_REF}" release  > /dev/null 2>&1'
            //'git tag v' + uikitConfig.getReleaseNameWithBuildNum()
            //'git push --force --quiet "https://${GH_TOKEN}@${GH_REF} v" ' + uikitConfig.getVersionWithBuildNumber() + ' > /dev/null 2>&1'
          ].join('&&');
        }
      }
    },
